...
# BU refresh
# -----------------------------
def refresh_bu_shortages(conn: sqlite3.Connection, bu_path: Path) -> tuple[int, int, int]:
    ...
    for _, r in mains.iterrows():
        cur.execute(
            ...
            INSERT INTO main_line(
              ...
              due_date, dock_date, wo_qty, line_value, is_active
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
...
def main():
    ...
    try:
        ...
        mains_n, subs_n, pegs_n = refresh_bu_shortages(conn, BU_XLSX)
