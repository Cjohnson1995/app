...
with tabs[0]:
    ...
    # Optional: Prioritize SHORT FREE toggle
...
**Work Order:** {sel_row.get("parent_wo","") or "—"}  
    ...
    with colB:
        ...
        # Dynamic sub status (derived from Comp Remaining next-op)
        _status_next = subs["rem_ops"].apply(lambda x: pd.Series(derive_sub_status(x), index=["sub_status", "next_op"]))
        subs = pd.concat([subs, _status_next], axis=1)
        show = subs[
            [
                "sub_part",
                "sub_source",
